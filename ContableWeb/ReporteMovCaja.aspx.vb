...
Public Class ReporteMovCaja
    ...
    Public Property Tabla() As DataTable
        ...
        End Set
    End Property
    Private _tablaSubtotales As DataTable
    Public Property TablaSubtotales() As DataTable
        Get
            Return _tablaSubtotales
        End Get
        Set(ByVal value As DataTable)
            _tablaSubtotales = value
        End Set
    End Property
    ...
    Private Sub cmdVerMovimientos_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles cmdVerMovimientos.Click
        If ValidarForm() Then
            ...
            Me.TablaSubtotales = Sistema.VistaSubtotalesMovCaja(Me.txtFechaDesde.Text, Me.txtFechaHasta.Text)
            Me.ViewState("Tabla") = Me.Tabla
            Me.grillaMovimientos.DataSource = Me.Tabla
            Me.grillaMovimientos.DataBind()
            Me.grilla.DataSource = Me.TablaSubtotales
            Me.grilla.DataBind()
            lblTotal.Text &= "Totales - Ingresos:$"
            Dim ingresos As Double = 0
            Dim egresos As Double = 0
            For Each row As DataRow In Me.TablaSubtotales.Rows
                ingresos += row.Item("ingresos")
                egresos += row.Item("egresos")
            Next
            lblTotal.Text &= ingresos.ToString
            lblTotal.Text &= " - Egresos:$"
            lblTotal.Text &= egresos.ToString
            lblTotal.Text &= " - Saldo:$"
            lblTotal.Text &= (ingresos - egresos).ToString
        End If
    End Sub
